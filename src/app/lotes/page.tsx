...
export default function LotesPage() {
    ...
    const { data, isLoading, isError, error } = useLotes(pagina, limite);
        ...
        <main className="p-6">
            ...
            <table className="min-w-full border-collapse border">
                ...
                <tbody>
                    {data?.data.map((lote) => (
                        <tr key={lote.id}>
                            <td className="border p-2">{lote.numeroLote || "N/A"}</td>
                            <td className="border p-2">{lote.tonalidade}</td>
                            <td className="border p-2">{lote.bitola}</td>
                            <td className="border p-2">{lote.quantidadeAtual - lote.quantidadeReservada}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
